export default {
  actions: {
    ...
    resetFiltering(ctx) {
      ctx.commit('RESET_FILTERING');
    },
    setFormPart(ctx, value) {
      ctx.commit('SET_FORM_PART', value);
    },
    resetForm(ctx) {
      ctx.commit('RESET_FORM');
    },
  },
  mutations: {
    ...
    TOGGLE_PART(state, payload) {
      state.partsList.map((el) => {
        if (el.partId === payload) {
          const newElem = el;
          newElem.isDeleted = !newElem.isDeleted;
          return newElem;
        }
        return el;
      });
      localStorage.setItem('partsList', JSON.stringify(state.partsList));
    },
    UPDATE_PART(state, payload) {
      let mappedParts = [];
      let newPart = {};

      const part = state.partsList.find((el) => (
        el.partId === payload.partId
      ));

      if (part) {
        newPart = Object.assign(part, payload);
        newPart.createdAt = part.createdAt;
        mappedParts = state.partsList.filter((el) => (el.partId !== payload.partId));
      } else {
        newPart = payload;
        mappedParts = state.partsList;
      }

      mappedParts.push(newPart);
      state.partsList = mappedParts.sort((a, b) => {
        if (state.sorting.desc) {
          return state.sorting.param === 'createdAt' ? new Date(b[state.sorting.param]).getTime() - new Date(a[state.sorting.param]).getTime()
            : b[state.sorting.param] - a[state.sorting.param];
        }
        return state.sorting.param === 'createdAt' ? new Date(a[state.sorting.param]).getTime() - new Date(b[state.sorting.param]).getTime()
          : a[state.sorting.param] - b[state.sorting.param];
      });
    ...
    SET_SORTING(state, payload) {
      if (state.sorting.param === payload.param) {
        state.sorting.desc = !payload.desc;
      } else {
        state.sorting = payload;
        state.sorting.desc = true;
      }

      localStorage.setItem('listSorting', JSON.stringify(state.sorting));
    ...
    RESET_FILTERING(state) {
      state.filtering = {};
      localStorage.removeItem('listFiltering');
    },
    SET_FORM_PART(state, payload) {
      state.editedPart = payload;
    },
    RESET_FORM(state) {
      state.editedPart = {
        partId: null,
  ...
  getters: {
    ...
    availableParts(state) {
      return state.partsList.filter((el) => (
        !el.isDeleted ? el : null
      ));
    },
    deletedParts(state) {
      return state.partsList.filter((el) => (
    ...
    manufacturerIds(state) {
      const { partsList } = state;
