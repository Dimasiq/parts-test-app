  ...
  actions: {
    fetchPartsList(ctx) {
      if (localStorage.listSorting) {
        ctx.commit('setSorting', JSON.parse(localStorage.getItem('listSorting')));
        console.log('fetched sorting from localStorage');
      } else {
        ctx.commit('setSorting', ctx.state.defaultSorting);
      }

      if (localStorage.partsList) {
        ctx.commit('updatePartsList', JSON.parse(localStorage.getItem('partsList')));
      ...
      ctx.commit('updatePartsList', res.data);
      console.log('fetched from server');
      return res.data;
    },
    updatePart(ctx, data) {
      ctx.commit('updatePart', data);
    },
    changeActiveTab(ctx, value) {
      ctx.commit('changeActiveTab', value);
    },
    setSorting(ctx, value) {
      ctx.commit('setSorting', value);
    },
  },
  mutations: {
    updatePartsList(state, payload) {
